<?php
	...
	switch($_POST['action']) {
		case 'addCustomer':
			...
			$statement = $database->prepare('INSERT INTO user (name, email, password) VALUES(:name, :email, :password)');
		...
		case 'getCustomer':
			...
			$statement = $database->prepare("SELECT * FROM user WHERE role = 0 AND status = :status ORDER BY {$order} {$sort} LIMIT :offset, :limit;");
			...
			if($statement->execute()) {
				...
				$statement = $database->prepare('SELECT COUNT(user_id) FROM user WHERE role = 0 AND status = :status LIMIT 1');
				$statement->bindParam(':status', $_POST['status'], PDO::PARAM_INT);
				$statement->execute();
			...
			break;
		case 'blockCustomer':
			$statement = $database->prepare('UPDATE user SET status = :status, updation_date = NOW() WHERE user_id = :id AND status != 1');
